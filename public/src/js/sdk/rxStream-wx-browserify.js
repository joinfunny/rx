...
},{"./JSON":1,"./config":2,"./jMd5":5,"./store":7,"./utils":9}],4:[function(require,module,exports){
var _ = require('./utils');
var JSON = require('./JSON');
...
module.exports = dom;
},{"./JSON":1,"./utils":9}],5:[function(require,module,exports){
...
rxStream.userIdentify = function (id, props, isSave) {
  if (typeof id === 'undefined') {
...
rxStream.trackSignup = function (id, event, props) {
  props = props || { subject: {}, object: {}, properties: {} };
  if (core.check({
    userId: id,
    event: event,
    properties: props.properties || {},
...
rxStream.getSessionId = function () {
  return store.getSessionId();
};

rxStream.getUtils=function(){
  return _;
};

rxStream.getUADetector=function(){
  return UADetector;
};

rxStream.preInit = function(){

};

rxStream.inited = function(){
...
},{"./JSON":1,"./UADetector":8,"./config":2,"./core":3,"./dom":4,"./store":7,"./utils":9}],7:[function(require,module,exports){
var config = require('./config');
var _ = require('./utils');
var JSON=require('./JSON');
...
},{"./JSON":1,"./config":2,"./utils":9}],8:[function(require,module,exports){
...
},{"./JSON":1}],9:[function(require,module,exports){
var config = require('./config');
var JSON = require('./JSON');
...
_.info = {
  ...
  properties: function () {
    ...
    return _.extend(_.generateNewProperties({
      ...
      //b_dollar_device: detector.device.name,
      //b_dollar_device_version: detector.device.fullVersion
    }), {
...
module.exports = _;
},{"./JSON":1,"./config":2,"./uaDetector":8}],10:[function(require,module,exports){
var _ = require('../utils');
...
},{"../JSON":1,"../utils":9,"./rxStream":11}],11:[function(require,module,exports){
var _ = require('../utils');
var store = require('../store');
var core = require('../core');
var dom = require('../dom');
...
_.extend(rxStream, {
  ...
  inited: function () {
    try {
      ...
      //初始化来源设备用户和时间戳
      if (url.indexOf("RXUID") != -1) {
        var referUniqueID = _.getQueryParam(url, "RXUID");
        var referTimeStamp = _.getQueryParam(url, "RXTS");
        store.set("WX_REFER_UID", referUniqueID);
        store.set("WX_REFER_TIMESTAMP", referTimeStamp);
      }
      var from;
      //获取来源渠道
      if (url.indexOf("from") != -1) {
        from = _.getQueryParam(url, "from") || 'unknown';

      }

      var i = document.referrer;
      if (i.indexOf("mp.weixin.qq.com") != -1 || i.indexOf("mp.weixinbridge.com") != -1) {
        from = 'wexin';
      }

      store.set("WX_FROM_TYPE", fromTypes[from]);

      //获取来源的微信用户信息，加载到当前页面
      _.each(WXUserArr, function (item) {
        var value = _.getQueryParam(url, item[0]);
        if (value) {
          var objectProps = {};
          objectProps[item[1]] = value;
          _
            .info
            .registerObject(objectProps);
        }
      });

    } catch (d) {}
  },
  wxInitTask: function (taskId) {
    _
      .info
      .register({"WX_TASK_ID": taskId});
  },
  //自定义代码：浏览页面
  wxViewPage: function (taskId) {
    var state = store.get();
    this.wxInitTask(taskId);
    this.track("wx_view_page", {
      properties: {
        b_page_referrer: document.referrer,
        b_page_referrer_host: _.info.referringDomain(document.referrer),
        b_page_url: location.href,
        b_page_url_path: location.pathname,
        taskId: _.info.currentProps.taskId, //对应的任务ID
        fromId: state['WX_REFER_UID'], //来源用户的标识，对应的是数据为uniqueId,必传参数
        fromType: state['WX_FROM_TYPE'], //分享来源类型,必传参数
        timeStamp: config.loadTime //浏览时间,必传参数
      },
      subject: {},
      object: {}
    });
  ...
  /**
   * 包装分享的连接
   * url:http://stream.ruixuesoft.com?RXUID=&RXTS=&OI=&NN=&SX=&CT=&PVC=&CTY=&HIU=&UI=
   * RXUID--uniqueId
   * RXTS--timeStamp
   * OI:openId
   * NN:nickName
   ...
   * UI:unionid
   */
  wrapShareLink: function (url) {
    var uniqueId = store.getDeviceId();
    var subject = store.getSessionSubject() || store.getSubject();
    var returnLink = url;
    try {
      url = _.concatQueryParam(url, "RXUID", uniqueId);
      url = _.concatQueryParam(url, "RXTS", config.loadTime);
      if (subject) {
        _
          .each(WXUserArr, function (item) {
            url = subject[item[1]]
              ? _.concatQueryParam(url, item[0], subject[item[1]])
              : url;
          });
      }
      return url;
  ...
  /**
   * 标识当前网页授权获得的微信用户信息
   * @param wxUserInfo 获取到的微信用户信息对象
   */
  wxUserInfoIdentify: function (wxUserInfo) {
    if (_.isEmptyObject(wxUserInfo)) {
      _.log('输入参数必须是一个非空的微信用户对象');
      return;
    }
    this.userIdentify(wxUserInfo.openid, wxUserInfo, true);
  },
  /**
   * 分享到朋友圈
   */
  trackShareTimeLine: function () {
    this._trackWxShare(fromTypes.timeline);
  },
  /**
   * 分享到群
   */
  trackShareGroupMessage: function () {
    this._trackWxShare(fromTypes.groupmessage);
  },
  /**
   * 分享给单个朋友
   */
  trackShareSingleMessage: function () {
    this._trackWxShare(fromTypes.singlemessage);
  },
  _trackWxShare: function (fromType) {
    this.track("wx_share", {
      properties: {
        taskId: _.info.currentProps.taskId, //对应的任务ID
        fromId: store.get('WX_REFER_UID'), //来源用户的标识，对应的是数据为uniqueId,必传参数
        shareType: fromType, //分享到的渠道类型,听wx_view_page的fromType,必传参数
        timeStamp: config.loadTime //浏览时间,必传参数
      }
    });
  }
});
},{"../JSON":1,"../UADetector":8,"../config":2,"../core":3,"../dom":4,"../rxStream":6,"../store":7,"../utils":9}]},{},[10]);
